#  This is bvt_activities_fetchActivitiesByDate.py
#
...
# TEST TARGETS:     ActivityService.fetchActivitiesByDate(dateType, beginTime, endTime, lastUUID)
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_activities_fetchActivitiesByDate"
...
def BvtActivitiesFetchActivitiesByDate():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for fetching activities By Date using the Activity service MBean"
...
#  Get the activity sort key
   ...
   print ""
   print "ActivityService.fetchActivitiesByDate(dateType, beginTime, endTime, lastUUID)" 
   print "Fetches activities, either created or modified within the specified dates..."
   print "This should list all Activities created between 01/01/09 thru 12/31/09." 
   print "Should return 4 activities, including any in the Trash folder."
   allbydate = ActivityService.fetchActivitiesByDate("created", "2009.01.01", "2009.12.31", "")
   print ""
   
#  Sort the returned activities
   strUtil = com.ibm.janet.core.StringUtils(Boolean.TRUE)
   sortedActivities = strUtil.sortHashTableVectorByKey( allbydate, activitySortKey, 0)
   ...
   BvtActivitiesCommon.writeActivitiesToFile(bvtActivitiesCommon, RUNFILE, sortedActivities, flags )
...
bvtActivitiesCommon=BvtActivitiesCommon()
print "BVT Fetch Activities By Date test ... starting."

#-------------------------------------------------
# Main  - just execute the Activity test function
#-------------------------------------------------
print "BVT Fetch Activities Test ... calling BvtActivitiesFetchActivitiesByDate()"
if DEBUG_SCRIPT != None:
   ...
   BvtActivitiesFetchActivitiesByDate()
   global fErrorFile
   fErrorFile.close()
   os.unlink(ERRORFILE)
else:	
