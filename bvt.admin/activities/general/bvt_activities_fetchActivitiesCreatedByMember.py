#  This is bvt_activities_fetchActivitiesCreatedByMember.py
#
...
# TEST TARGETS:     ActivityService.fetchActivitiesCreatedByMember(member)
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_activities_fetchActivitiesCreatedByMember"
...
def BvtActivitiesFetchActivitiesCreatedByMember():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for fetching activities created by a specified member using the Activity service MBean"
   ...
   print ""
   print "ActivityService.fetchActivitiesCreatedByMember(member) fetches ALL activities created by the specified member..."
   print ""
   ...
   aj70created = ActivityService.fetchActivitiesCreatedByMember(aj70)
   print ""
   
   # Sort the returned activities
   strUtil = com.ibm.janet.core.StringUtils(Boolean.TRUE)
   sortedActivities = strUtil.sortHashTableVectorByKey( aj70created, activitySortKey, 0)
...
bvtActivitiesCommon=BvtActivitiesCommon()
print "BVT Fetch Activities Created By Member test ... starting."

#-------------------------------------------------
# Main  - just execute the Activity test function
#-------------------------------------------------
print "BVT Fetch Activities Test ... calling BvtActivitiesFetchActivitiesCreatedByMember()"
if DEBUG_SCRIPT != None:
   ...
   BvtActivitiesFetchActivitiesCreatedByMember()
   global fErrorFile
   fErrorFile.close()
   os.unlink(ERRORFILE)
else:	
