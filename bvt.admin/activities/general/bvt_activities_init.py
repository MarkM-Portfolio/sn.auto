...
# TEST TARGETS:     Fetch all Activities on the server, put them into Trash folder, then Purge them from database
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_activities_init"
...
def BvtActivitiesInit():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for removing all existing activities off the server - using MBean commands."
   ...
   print ""
   print "Fetching all activities on the server..."
   all=ActivityService.fetchActivities()
   print ""
   print "Deleting all the activities, so that they are in the trash folder"
   ActivityService.deleteActivities(all)
   print ""
   print "Fetching all the Activities in the Trash Folder"
   alltrash=TrashCollectionService.fetchTrash()
   print ""
   print "Purging all the activities from the Trash folder and from the database tables"
   TrashCollectionService.purgeTrash(alltrash)
   print ""
   print "Now verify that there are NO activities on the server - do a fetchActivities command"
   activities = ActivityService.fetchActivities()
   print ""
   print "List All Activities on the server - should be none left."
   print ""
   ...
   print ""
   print "Now verify there are NO activities left in the trash - do a fetchTrash command"
   print ""
   trashact=TrashCollectionService.fetchTrash()
   print ""
   print "List All Activities in the Trash - should be none left."
   print ""

   count = BvtActivitiesCommon.listAllActivities(bvtActivitiesCommon, trashact)
   print "%d Trashed Activities retrieved." % count

   flags = Hashtable()
   
   BvtActivitiesCommon.writeActivitiesToFile(bvtActivitiesCommon, RUNFILE, trashact, flags )
...
bvtActivitiesCommon=BvtActivitiesCommon()
print "BVT Activities Initialization Test ... starting."

#-------------------------------------------------
# Main  - just execute the Activity test function
#-------------------------------------------------
print "BVT Activities Initialization Test ... removes all Activities from server & from Trash"
