...
# TEST TARGETS:     Run the MemberService.fetchMembers mbean command
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_activities_fetchMembers"
...
def BvtMembersInit():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for MemberService.fetchMembers(Amy Jones7*)MBean command."
   ...
   #Get the members sort key (used to sort the members vector that is returned from the cmd)
   membersSortKey = BvtCommon.getProperty(bvtCommon, configProps, "members_sort_key")


   print ""
   print "Issuing the command MemberService.fetchMembers('Amy Jones7*')..."
   print "The command should return info about Amy Jones7, and 70-79..."
   members=MemberService.fetchMembers("Amy Jones7*")
   print ""
   
   #Sort the returned members
   strUtil = com.ibm.janet.core.StringUtils(Boolean.TRUE)
   sortedMembers = strUtil.sortHashTableVectorByKey( members, membersSortKey, 0)
   print "Members Sorted Ascending Alphabetically by Key: " + membersSortKey + " ..."
   count= BvtMembersCommon.listAllMembers(bvtMembersCommon, sortedMembers)               
   ...
   BvtMembersCommon.writeMembersToFile(bvtMembersCommon, RUNFILE, sortedMembers, flags)
