#  bvt_activities_fetch_metrics.py
#
#  Jython Script to test some Activity commands on IBM Activities Enterprise Application
#  (C) Copyright IBM Corporation, 2008
#
#  DESCRIPTION:
#  This script retrieves all metrics associated with activities on the target server.
#
...
# TEST TARGETS:     ActivityMetricsService.fetchMetrics()
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_activities_fetch_metrics"
...
SNAPSHOTFILE = "activitiesMetricsSnapshot.txt"
METRIC_NAME_PREFIX = "metric.keyname"
METRIC_UPDATED_NAME_PREFIX = "activities.metric."
...
def BvtActivitiesFetchMetrics():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for fetching all Activities Metrics using the ActivityMetricsService MBean"
   ...
   ActivityMetricsService.saveMetricToFile("", 0, "metrics.cache.timeout.in.minutes")
   ...
   while enumKeys.hasMoreElements():
      ...
      print "DEBUG: Assigned Baseline value of: %s for Metric: %s to Activities Metrics Baseline hashtable..." % (metricsUpdateKeys.get(key), key)

   print ""
   print "Retrieving Current Activities Metrics Snapshot from Snapshot File: " + SNAPSHOTFILE + " ..."
   ...
   for metric in metricSnapshotList:
	...
	value = value.strip()
	print "Adding Metric: " + key + " with value: " + value + " to Activities Metrics snapshot hashtable..."
   	if value.find(".") > 0:
   ...
   print ""
   print "Fetching Current Activites Metrics on Target Server..."
   activitiesMetrics = ActivityMetricsService.fetchMetrics()   
   print ""
   
   print ""
   print "Fetched the following current Activities Metrics Data . . ."
   bvtMetricsCommon.listAllMetrics(activitiesMetrics)
   print ""
   
   print ""
   print "Checking Retrieved Metrics For Errors . . ."
   metricsErrors = bvtMetricsCommon.checkMetricsForErrors(activitiesMetrics)
   print ""
   ...
   if( htMetricSnapshot.size() != activitiesMetrics.size() ):
	raise "ERROR: Metrics Snapshot Size: %d does not equal Fetch Metrics Size: %d" % ( htMetricSnapshot.size(), activitiesMetrics.size() ) 
   ...
   if( len(metricsKeys) != activitiesMetrics.size() ):
	raise "ERROR: Metrics Key Size: %d read in from properties file does not equal Fetch Metrics Size: %d" % ( metricsKeys.size(), activitiesMetrics.size() ) 
   ...
   while keysIterator.hasNext():
      key = keysIterator.next()
      value = activitiesMetrics.get(key)
      if type(value).__name__ == 'org.python.core.PyFloat':
         actualResult = float(activitiesMetrics.get(key))
         ...
         actualResult = activitiesMetrics.get(key)
...
bvtMetricsCommon=BvtMetricsCommon()
print "BvtActivitiesFetchMetrics tests ... starting."

#-------------------------------------------------
# Main  - just execute the BvtActivitiesFetchMetrics test function
#-------------------------------------------------
print "BVT BvtActivitiesFetchMetrics Test ... calling BvtActivitiesFetchMetrics()"
if DEBUG_SCRIPT != None:
   ...
   try:
      ...
      ActivityMetricsService.saveMetricToFile("", 60, "metrics.cache.timeout.in.minutes")
   else:
      global fErrorFile
