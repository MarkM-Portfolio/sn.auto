#  This is bvt_activities_SchedulerStartStop.py
#
...
# TEST TARGETS:     Scheduler.stop(schedName); Scheduler.start(schedName); Scheduler.isRunning(schedName)
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_activities_SchedulerStartStop"
...
def BvtActivitiesSchedulerStartStop():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script that will test the Scheduler start, stop, & isRunning MBean commands"
...
#   activitySortKey = BvtCommon.getProperty(bvtCommon, configProps, "activity_sort_key")
   ...
   print " *******************************************"
   print "Initial test for the ISRUNNING cmd: Scheduler.isRunning(local)"
   print "This command should tell us the local scheduler IS running."
   running = Scheduler.isRunning("local")
   ...
   BvtSchedulerCommon.writeStringToFile(bvtSchedulerCommon, RUNFILE, running, flags)    

   print " *******************************************"
   print "Will now test the STOP cmd: Scheduler.stopJob(local)"
   print "This command will stop the local scheduler from running."
   stopped = Scheduler.stop("local")
   print "This is what is returned from the command:"
   print stopped
   ...
   BvtSchedulerCommon.writeStringToFile(bvtSchedulerCommon, RUNFILE, stopped, flags)


   print " *******************************************"
   print "Now testing the ISRUNNING cmd: Scheduler.isRunning(local)"
   print "This command should tell us the local scheduler is NOT running."
   notrunning = Scheduler.isRunning("local")
   print "This is what is returned from the command:"
   print notrunning
   ...
   flags = Hashtable()
   flags.put("append", "true")
   BvtSchedulerCommon.writeStringToFile(bvtSchedulerCommon, RUNFILE, notrunning, flags)


   print " *******************************************"
   print "Will now test the START cmd: Scheduler.startJob(local)"
   print "This command will start up the local scheduler."
   started = Scheduler.start("local")
   print "This is what is returned from the command:"
   print started
   ...
   flags = Hashtable()
   flags.put("append", "true")
   BvtSchedulerCommon.writeStringToFile(bvtSchedulerCommon, RUNFILE, started, flags)


   print " *******************************************"
   print "Run the ISRUNNING cmd again: Scheduler.isRunning(local)"
   print "This time it should tell us the local scheduler IS running."
   running2 = Scheduler.isRunning("local")
   print "This is what is returned from the command:"
   print running2
   ...
   flags = Hashtable()
   flags.put("append", "true")
   BvtSchedulerCommon.writeStringToFile(bvtSchedulerCommon, RUNFILE, running2, flags)
...
bvtSchedulerCommon=BvtSchedulerCommon()
print "BVT testing of Scheduler start, stop, & isRunning commands ... starting."

#-------------------------------------------------
# Main  - just execute the Scheduler start, stop & isRunning functions
#-------------------------------------------------
print "BVT Scheduler start, stop, isRunning Tests ... calling BvtActivitiesSchedulerStartStop()"
if DEBUG_SCRIPT != None:
   ...
   BvtActivitiesSchedulerStartStop()
   global fErrorFile
   fErrorFile.close()
   os.unlink(ERRORFILE)
else:	
