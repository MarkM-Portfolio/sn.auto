...
# TEST TARGETS:     Run the CommunitiesService.createCommunity command
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_communities_createCommunity"
...
def BvtCreateCommunityInit():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for createCommunity, fetchCommByName and fetchMember MBean commands."
   ...
   dsmlFile = BvtCommon.getProperty(bvtCommon, configProps, "communities3.dsml.filename")
   ownerEmail = BvtCommon.getProperty(bvtCommon, configProps, "communities3.owner.email")
   oldCommName1 = BvtCommon.getProperty(bvtCommon, configProps, "communities3.old.name")
   currentDate = Date()
   oldCommName1 = oldCommName1 + " " + DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG).format(currentDate);
   print ""
   createCommCmd1 = "CommunitiesService.createCommunity(\"" + oldCommName1 + "\", \"" + ownerEmail + "\", 0, \"" + dsmlFile + "\")"   
   ...
   goldMemberTable = bvtCommunitiesCommon.loadPropSetsIntoHashTable("communities3", "member.login.", "member.role.")
...
bvtCommMembersCommon=BvtCommMembersCommon()
print "BVT CommunitiesService createCommunity Tests ... starting."

#-------------------------------------------------
# Main  - just execute the createCommunity test function
#-------------------------------------------------
print "BVT createCommunity, fetchCommByName and fetchMember Tests..."
