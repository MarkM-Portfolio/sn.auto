#  This is:  bvt_communities_createCommunityWithEmail.py
#
...
# TEST TARGETS:     Run the CommunitiesService.createCommunityWithEmail command
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_communities_createCommunityWithEmail"
...
def BvtCreateCommunityByEmailInit():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for createCommunityWithEmail, fetchCommByName and fetchMember MBean commands."
   ...
   dsmlFile = BvtCommon.getProperty(bvtCommon, configProps, "communities2.dsml.filename")
   ownerEmail = BvtCommon.getProperty(bvtCommon, configProps, "communities2.owner.email")
   oldCommName1 = BvtCommon.getProperty(bvtCommon, configProps, "communities2.old.name")
   currentDate = Date()
   oldCommName1 = oldCommName1 + " " + DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG).format(currentDate);
   print ""
   createCommCmd1 = "CommunitiesService.createCommunityWithEmail(\"" + oldCommName1 + "\", \"" + ownerEmail + "\", 1, \"" + dsmlFile + "\")"   
   ...
   goldMemberTable = bvtCommunitiesCommon.loadPropSetsIntoHashTable("communities2", "member.login.", "member.role.")
...
print "BVT CommunitiesService createCommunityByEmail Tests ... starting."

#-------------------------------------------------
# Main  - just execute the createCommunity test function
#-------------------------------------------------
print "BVT createCommunityByEmail, fetchCommByName and fetchMember Tests..."
