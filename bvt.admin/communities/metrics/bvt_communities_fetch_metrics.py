#  bvt_communities_fetch_metrics.py
#
#  Jython Script to test some Communities Metrics on IBM Communities Enterprise Application
#  (C) Copyright IBM Corporation, 2008
#
#  DESCRIPTION:
#  This script retrieves all metrics associated with communities on the target server.
#
...
# TEST TARGETS:     CommunitiesMetricsService.fetchMetrics()
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_communities_fetch_metrics"
...
SNAPSHOTFILE = "communitiesMetricsSnapshot.txt"
METRIC_NAME_PREFIX = "metric.keyname"
METRIC_UPDATED_NAME_PREFIX = "communities.metric."
...
def BvtCommunitiesFetchMetrics():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for fetching all Communities Metrics using the CommunitiesMetricsService MBean"
   ...
   CommunitiesMetricsService.saveMetricToFile("", 0, "metrics.cache.timeout.in.minutes")
   ...
   while enumKeys.hasMoreElements():
      ...
      print "DEBUG: Assigned Baseline value of: %s for Metric: %s to Communities Metrics Baseline hashtable..." % (metricsUpdateKeys.get(key), key)

   print ""
   print "Retrieving Current Communities Metrics Snapshot from Snapshot File: " + SNAPSHOTFILE + " ..."
   ...
   for metric in metricSnapshotList:
	...
	value = value.strip()
	print "Adding Metric: " + key + " with value: " + value + " to Communities Metrics snapshot hashtable..."
   	if value.find(".") > 0:
   ...
   print ""
   print "Fetching Current Communities Metrics on Target Server..."
   communitiesMetrics = CommunitiesMetricsService.fetchMetrics()   
   print ""
   
   print ""
   print "Fetched the following current Communities Metrics Data . . ."
   bvtMetricsCommon.listAllMetrics(communitiesMetrics)
   print ""
   
   print ""
   print "Checking Retrieved Metrics For Errors . . ."
   metricsErrors = bvtMetricsCommon.checkMetricsForErrors(communitiesMetrics)
   print ""
   ...
   if( htMetricSnapshot.size() != communitiesMetrics.size() ):
	raise "ERROR: Metrics Snapshot Size: %d does not equal Fetch Metrics Size: %d" % ( htMetricSnapshot.size(), communitiesMetrics.size() ) 
   ...
   if( len(metricsKeys) != communitiesMetrics.size() ):
	raise "ERROR: Metrics Key Size: %d read in from properties file does not equal Fetch Metrics Size: %d" % ( metricsKeys.size(), communitiesMetrics.size() ) 
   ...
   keys = communitiesMetrics.keySet()
   ...
   while keysIterator.hasNext():
      key = keysIterator.next()
      value = communitiesMetrics.get(key)
      if type(value).__name__ == 'org.python.core.PyFloat':
         actualResult = float(communitiesMetrics.get(key))
         ...
         actualResult = communitiesMetrics.get(key)
...
bvtMetricsCommon=BvtMetricsCommon()
print "BvtCommunitiesFetchMetrics tests ... starting."

#-------------------------------------------------
# Main  - just execute the BvtCommunitiesFetchMetrics test function
#-------------------------------------------------
print "BvtCommunitiesFetchMetrics Test ... calling BvtCommunitiesFetchMetrics()"
if DEBUG_SCRIPT != None:
   ...
   try:
      ...
      CommunitiesMetricsService.saveMetricToFile("", 60, "metrics.cache.timeout.in.minutes")
   else:
      global fErrorFile
