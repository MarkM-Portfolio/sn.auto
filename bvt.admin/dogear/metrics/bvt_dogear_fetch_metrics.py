#  bvt_dogear_fetch_metrics.py
#
#  Jython Script to test some Dogear commands on IBM Dogear Enterprise Application
#  (C) Copyright IBM Corporation, 2008
#
#  DESCRIPTION:
#  This script retrieves all metrics associated with dogear on the target server.
#
...
# TEST TARGETS:     DogearMetricsService.fetchMetrics()
...
execfile('dogearAdmin.py') # only do this once
execfile('bvt_common.py')
execfile('bvt_metrics_common.py')
execfile('ListServiceModule.py')

#Globals
UNIQUE_SCRIPT_ID = "bvt_dogear_fetch_metrics"
...
SNAPSHOTFILE = "dogearMetricsSnapshot.txt"
METRIC_NAME_PREFIX = "metric.keyname"
METRIC_UPDATED_NAME_PREFIX = "dogear.metric."
...
def BvtDogearFetchMetrics():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for fetching all Dogear Metrics using the DogearMetricsService MBean"
   ...
   DogearMetricsService.saveMetricToFile("", 0, "metrics.cache.timeout.in.minutes")
   ...
   while enumKeys.hasMoreElements():
      ...
      print "DEBUG: Assigned Baseline value of: %s for Metric: %s to Dogear Metrics Baseline hashtable..." % (metricsUpdateKeys.get(key), key)

   print ""
   print "Retrieving Current Dogear Metrics Snapshot from Snapshot File: " + SNAPSHOTFILE + " ..."
   ...
   for metric in metricSnapshotList:
	...
	value = value.strip()
	print "Adding Metric: " + key + " with value: " + value + " to Dogear Metrics snapshot hashtable..."
   	if value.find(".") > 0:
   ...
   print ""
   print "Fetching Current Dogear Metrics on Target Server..."
   dogearMetrics = DogearMetricsService.fetchMetrics()   
   print ""
   
   print ""
   print "Fetched the following current Dogear Metrics Data . . ."
   bvtMetricsCommon.listAllMetrics(dogearMetrics)
   print ""
   
   print ""
   print "Checking Retrieved Metrics For Errors . . ."
   metricsErrors = bvtMetricsCommon.checkMetricsForErrors(dogearMetrics)
   print ""
   ...
   if( htMetricSnapshot.size() != dogearMetrics.size() ):
	raise "ERROR: Metrics Snapshot Size: %d does not equal Fetch Metrics Size: %d" % ( htMetricSnapshot.size(), dogearMetrics.size() ) 
   ...
   if( len(metricsKeys) != dogearMetrics.size() ):
	raise "ERROR: Metrics Key Size: %d read in from properties file does not equal Fetch Metrics Size: %d" % ( metricsKeys.size(), dogearMetrics.size() ) 
   ...
   while keysIterator.hasNext():
      key = keysIterator.next()
      value = dogearMetrics.get(key)
      if type(value).__name__ == 'org.python.core.PyFloat':
         actualResult = float(dogearMetrics.get(key))
         ...
         actualResult = dogearMetrics.get(key)
...
bvtMetricsCommon=BvtMetricsCommon()
print "BvtDogearFetchMetrics tests ... starting."

#-------------------------------------------------
# Main  - just execute the BvtDogearFetchMetrics test function
#-------------------------------------------------
print "BVT BvtDogearFetchMetrics Test ... calling BvtDogearFetchMetrics()"
if DEBUG_SCRIPT != None:
   ...
   try:
      ...
      DogearMetricsService.saveMetricToFile("", 60, "metrics.cache.timeout.in.minutes")
   else:
      global fErrorFile
