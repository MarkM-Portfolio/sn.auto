#  bvt_homepage_fetch_metrics.py
#
#  Jython Script to test some Homepage commands on IBM Homepage Enterprise Application
#  (C) Copyright IBM Corporation, 2008
#
#  DESCRIPTION:
#  This script retrieves all metrics associated with homepage on the target server.
#
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_homepage_fetch_metrics"
...
METRIC_UPDATED_NAME_PREFIX = "homepage.metric."
...
def BvtHomepageFetchMetrics():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for fetching all Homepage Metrics using the HomepageMetricsService MBean"
   ...
   HomepageMetricsService.saveMetricToFile("", 0, "metrics.cache.timeout.in.minutes")
   ...
   while enumKeys.hasMoreElements():
      ...
      print "DEBUG: Assigned Baseline value of: %s for Metric: %s to Homepage Metrics Baseline hashtable..." % (metricsUpdateKeys.get(key), key)

   print ""
   print "Retrieving Current Homepage Metrics Snapshot from Snapshot File: " + SNAPSHOTFILE + " ..."
   ...
   for metric in metricSnapshotList:
	...
	value = value.strip()
	print "Adding Metric: " + key + " with value: " + value + " to Homepage Metrics snapshot hashtable..."
   	if value.find(".") > 0:
   ...
   print ""
   print "Fetching Current Homepage Metrics on Target Server..."
   homepageMetrics = HomepageMetricsService.fetchMetrics()   
   print ""
   
   print ""
   print "Fetched the following current Homepage Metrics Data . . ."
   bvtMetricsCommon.listAllMetrics(homepageMetrics)
   print ""
   
   print ""
   print "Checking Retrieved Metrics For Errors . . ."
   metricsErrors = bvtMetricsCommon.checkMetricsForErrors(homepageMetrics)
   print ""
   ...
   if( htMetricSnapshot.size() != homepageMetrics.size() ):
	raise "ERROR: Metrics Snapshot Size: %d does not equal Fetch Metrics Size: %d" % ( htMetricSnapshot.size(), homepageMetrics.size() ) 
   ...
   if( len(metricsKeys) != homepageMetrics.size() ):
	raise "ERROR: Metrics Key Size: %d read in from properties file does not equal Fetch Metrics Size: %d" % ( metricsKeys.size(), homepageMetrics.size() ) 
   ...
   while keysIterator.hasNext():
      key = keysIterator.next()
      value = homepageMetrics.get(key)
      if type(value).__name__ == 'org.python.core.PyFloat':
         actualResult = float(homepageMetrics.get(key))
         ...
         actualResult = homepageMetrics.get(key)
...
bvtMetricsCommon=BvtMetricsCommon()
print "BvtHomepageFetchMetrics tests ... starting."

#-------------------------------------------------
# Main  - just execute the BvtHomepageFetchMetrics test function
#-------------------------------------------------
print "BVT BvtHomepageFetchMetrics Test ... calling BvtHomepageFetchMetrics()"
if DEBUG_SCRIPT != None:
   ...
   try:
      ...
      HomepageMetricsService.saveMetricToFile("", 60, "metrics.cache.timeout.in.minutes")
   else:
      global fErrorFile
