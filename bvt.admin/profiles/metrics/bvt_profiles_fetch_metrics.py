#  bvt_profiles_fetch_metrics.py
#
#  Jython Script to test some Profiles commands on IBM Profiles Enterprise Application
#  (C) Copyright IBM Corporation, 2008
#
#  DESCRIPTION:
#  This script retrieves all metrics associated with profiles on the target server.
#
...
# TEST TARGETS:     ProfilesMetricsService.fetchMetrics()
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_profiles_fetch_metrics"
...
SNAPSHOTFILE = "profilesMetricsSnapshot.txt"
METRIC_NAME_PREFIX = "metric.keyname"
METRIC_UPDATED_NAME_PREFIX = "profiles.metric."
...
def BvtProfilesFetchMetrics():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for fetching all Profiles Metrics using the ProfilesMetricsService MBean"
   ...
   ProfilesMetricsService.saveMetricToFile("", 0, "metrics.cache.timeout.in.minutes")
   ...
   while enumKeys.hasMoreElements():
      ...
      print "DEBUG: Assigned Baseline value of: %s for Metric: %s to Profiles Metrics Baseline hashtable..." % (metricsUpdateKeys.get(key), key)

   print ""
   print "Retrieving Current Profiles Metrics Snapshot from Snapshot File: " + SNAPSHOTFILE + " ..."
   ...
   for metric in metricSnapshotList:
	...
	value = value.strip()
	print "Adding Metric: " + key + " with value: " + value + " to Profiles Metrics snapshot hashtable..."
   	if value.find(".") > 0:
   ...
   print ""
   print "Fetching Current Profiles Metrics on Target Server..."
   profilesMetrics = ProfilesMetricsService.fetchMetrics()   
   print ""
   
   print ""
   print "Fetched the following current Profiles Metrics Data . . ."
   bvtMetricsCommon.listAllMetrics(profilesMetrics)
   print ""
   
   print ""
   print "Checking Retrieved Metrics For Errors . . ."
   metricsErrors = bvtMetricsCommon.checkMetricsForErrors(profilesMetrics)
   print ""
   ...
   if( htMetricSnapshot.size() != profilesMetrics.size() ):
	raise "ERROR: Metrics Snapshot Size: %d does not equal Fetch Metrics Size: %d" % ( htMetricSnapshot.size(), profilesMetrics.size() ) 
   ...
   if( len(metricsKeys) != profilesMetrics.size() ):
	raise "ERROR: Metrics Key Size: %d read in from properties file does not equal Fetch Metrics Size: %d" % ( metricsKeys.size(), profilesMetrics.size() ) 
   ...
   keys = profilesMetrics.keySet()
   keysIterator = keys.iterator()
   while keysIterator.hasNext():
      key = keysIterator.next()
      snapshotValue = str(htMetricSnapshot.get(key))
      baselineValue = str(htBaselineMetrics.get(key))
      #print "Debug: Processing Metric Key: " + key
      #print "Debug: Snapshot Value Data Type: " + type(snapshotValue).__name__
      #print "Debug: Baseline Value Data Type: " + type(baselineValue).__name__
      #print "Debug: Snapshot Value: " + snapshotValue
      #print "Debug: Baseline Value: " + baselineValue
   ...
   while keysIterator.hasNext():
      key = keysIterator.next()
      value = profilesMetrics.get(key)
      if type(value).__name__ == 'org.python.core.PyFloat':
         actualResult = float(profilesMetrics.get(key))
         ...
         actualResult = profilesMetrics.get(key)
...
bvtMetricsCommon=BvtMetricsCommon()
print "BvtProfilesFetchMetrics tests ... starting."

#-------------------------------------------------
# Main  - just execute the BvtProfilesFetchMetrics test function
#-------------------------------------------------
print "BVT BvtProfilesFetchMetrics Test ... calling BvtProfilesFetchMetrics()"
if DEBUG_SCRIPT != None:
   ...
   try:
      ...
      ProfilesMetricsService.saveMetricToFile("", 60, "metrics.cache.timeout.in.minutes")
   else:
      global fErrorFile
