#  This is bvt_profiles_fetch_metrics_snapshot.py
#
#  Jython Script to test some Profiles commands on IBM Profiles Enterprise Application
#  (C) Copyright IBM Corporation, 2008
#
#  DESCRIPTION:
#  This script takes a snapshot of Profiles metrics and writes it to the file:
#  profilesMetricsSnapshot.txt.  The purpose of the snapshot file is to allow the 
...
#Globals
UNIQUE_SCRIPT_ID = "bvt_profiles_fetch_metrics_snapshot"
...
def BvtProfilesFetchMetricsSnapshot():
   print ""
   print "--------------------------------------------------------------------------------"
   print " BVT Test Script for Producing an Profiles Metrics Snapshot at the time this script is invoked "
   ...
   print ""
   print "Fetching Profiles Metrics Snapshot . . ."
   profilesMetricsSnapshot = ProfilesMetricsService.fetchMetrics()   
   print ""

   print ""
   print "Profiles Metrics Snapshot Data . . ."
   bvtMetricsCommon.listAllMetrics(profilesMetricsSnapshot)
   print ""
   
   print ""
   print "Writing Profiles Metrics Snapshot Data to snapshot file: " + SNAPSHOTFILE + " . . ."
   bvtMetricsCommon.writeMetricsToFile(SNAPSHOTFILE, profilesMetricsSnapshot, None)
   print ""
...
bvtMetricsCommon=BvtMetricsCommon()
print "BVT BvtProfilesFetchMetricsSnapshot ... starting."

#-------------------------------------------------
# Main  - just execute the BvtProfilesFetchMetricsSnapshot test function
#-------------------------------------------------
print "BVT BvtProfilesFetchMetricsSnapshot ... calling BvtProfilesFetchMetricsSnapshot()"
if DEBUG_SCRIPT != None:
   ...
   BvtProfilesFetchMetricsSnapshot()
   global fErrorFile
   fErrorFile.close()
   os.unlink(ERRORFILE)
else:	
