...
public class Smoke_RO_Ideation_Blogs extends SetUpMethods2 {
	
	private static Logger log = LoggerFactory.getLogger(Smoke_RO_Ideation_Blogs.class);
	...
	/**
	*<ul>
	*<li><B>Info: </B>Adding a Ideation Blog widget</li>
	*<li><B>Step: </B>Create a Community</li>
	*<li><B>Step: </B>Click Community Actions > Add Apps</li> 
	*<li><B>Step: </B>Add Ideation Blog widget</li>
	*<li><B>Verify: </B>Ideation Blog widget has been added</li> 
	*<li><B>Step: </B>Click Ideation Blog in the left side nav pane</li>
	*<li><B>Step: </B>Click Contribute an Idea button</li>
	*<li><B>Verify: </B>New Idea title field is present</li>
	*</ul>
	*/ 
	@Test(groups = {"smoke"})
	public void testUILoad() throws Exception{
		...
		log.info("INFO: Select Ideation Blogs from the left navigation menu");
		Community_LeftNav_Menu.IDEATIONBLOG.select(communitiesUI);
		ui.waitForSameTime();
		
		//Open Ideation blog
		log.info("INFO: Select the default ideation blog link");
		ui.clickLinkWithJavascript(ui.getCommIdeationBlogLink(new BaseCommunity.Builder(Data.getData().productionCommunityName).build()));
		
		ui.fluentWaitPresent(BlogsUIConstants.NewIdea);
		
		log.info("INFO: Goto New Idea");
		ui.gotoNewIdea();
		
		ui.fluentWaitTextPresent("New Idea");
