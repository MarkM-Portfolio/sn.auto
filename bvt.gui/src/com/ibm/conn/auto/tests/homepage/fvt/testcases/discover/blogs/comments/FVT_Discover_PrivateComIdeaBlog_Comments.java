...
public class FVT_Discover_PrivateComIdeaBlog_Comments extends SetUpMethodsFVT {
	...
	public void setUpClass() {
		...
		// User 1 create a restricted community and add the Ideation Blogs widget and add Users 2 and 3 as members (so as they can post comments successfully)
	...
	/**
	 * <ul>
	 * <li><B>Name:</B> PrivateComIdeaComment()</li>
	 * <li><B>How:</B> Use the API to create a private community owned by testUser1, add an idea blog, an Idea and a comment to the idea</li>
	 * <li><B>Step:</B> testUser1 log in to Communities</li>
	 * <li><B>Step:</B> testUser1 open a community you own with private access</li>
	 *<li><B>Step: testUser 1 add testUser 2 to the community as a member</B></li>
	 ...
	 * <li><B>Verify:</B> Verify that the news story for ideablog.comment.created is NOT seen</li>
	 * <li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/B372CFAF64A888C4852578760079EAA7">TTT - DISC - IDEABLOGS - 00040 - IDEABLOG.COMMENT.CREATED - PRIVATE COMMUNITY IDEABLOG (NEG)</a></li>	
	 * @author Naomi Pakenham
	 */	
	@Test(groups = {"fvtonprem", "fvtcloud"}, priority = 1)
	public void PrivateComIdeaComment() {
		...
		// Verify that the comment on idea event is NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{ideaCommentEvent, baseBlogPost.getContent(), baseBlogCommentUser3.getContent()}, TEST_FILTERS, false);
	...
	/**
	* <ul>
	* <li><B>Name:</B> PrivateComIdeaEditComment()()</li>
	...
	*<li><B>Verify: #1 - testUser 2 does NOT see two comments on the idea, one created by themselves, the other created by User 1</B></li>
	*<li><B>Verify: #2 - testUser 2 does NOT see 2 comments on the idea, one edited by themselves and one unedited by user 1</B></li>
	*<li><B>Verify: #3 - testUser 2 does NOT see two edited comments on the idea</B></li>
	*<li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/411FD00D49C25E3085257B500047CF26">TTT - DISC - IDEABLOGS - 00113 - IDEABLOG.COMMENT.EDITED - PRIVATE COMMUNITY IDEABLOG (NEG)</a></li>
	*</ul>	
	* @author Naomi Pakenham
	*/
	@Test(groups = {"fvtonprem", "fvtcloud"}, priority = 2)
	public void PrivateComIdeaEditComment() {
		...
		// Verify that the two comments on idea event and both User 1 and User 2's comments are NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{twoCommentsOnIdeaEvent, baseBlogPost.getContent(), blogCommentUser1.getContent(), blogCommentUser2.getContent()}, TEST_FILTERS, false);
		...
		// Verify that the two comments on idea event, User 1's comment and both User 2's original and updated comments are NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{twoCommentsOnIdeaEvent, baseBlogPost.getContent(), blogCommentUser1.getContent(), editedBaseBlogCommentUser2.getContent(), baseBlogCommentUser2.getContent()}, TEST_FILTERS, false);
		...
		// Verify that the two comments on idea event, User 1's original and updated comments and User 2's updated comment are NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{twoCommentsOnIdeaEvent, baseBlogPost.getContent(), baseBlogCommentUser1.getContent(), editedBaseBlogCommentUser1.getContent(), editedBaseBlogCommentUser2.getContent()}, TEST_FILTERS, false);
	...
	/**
	*<ul>
	*<li><B>Name: test_deleteCommentToIdea_PrivateCommunity()</B></li>
	*<li><B>Info: It can be assumed that each testcase starts with login and ends with logout and the browser being closed</B></li>
	*<li><B>Step: testUser 1 log in to Communities</B></li>
	*<li><B>Step: testUser 1 creates a new community with private access</B></li>
	*<li><B>Step: testUser 1 add testUser 2 to the community as a member</B></li>
	...
	*<li><B>Verify: #1 - testUser 2 does NOT see two comments on the idea, one created by themselves, the other created by User 1</B></li>
	*<li><B>Verify: #2 - testUser 2 does NOT see one comment on the idea, the comment author is testUser 1</B></li>
	*<li><B>Verify: #3 - testUser 2 does NOT see no comments on the idea</B></li>
	*<li><a HREF="Notes://Parallan/85257863004CBF81/088F49BF479C68258525751B0060FA97/A7C4B4B09FC5379F85257B50004796F1">TTT - DISC - IDEABLOGS - 00103 - IDEABLOG.COMMENT.DELETED - PRIVATE COMMUNITY IDEABLOG (NEG)</a></li>
	*</ul>
	* @author Patrick Doherty
	*/
	@Test(groups = {"fvtonprem", "fvtcloud"}, priority = 3)
	public void test_deleteCommentToIdea_PrivateCommunity() {
		...
		// Verify that the comment on the idea event is NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{twoCommentsOnIdeaEvent, baseBlogPost.getContent(), blogCommentUser1.getContent(), blogCommentUser2.getContent()}, TEST_FILTERS, false);
		...
		// Verify that the one comment event, two comments event, User 1's comment and User 2's deleted comment are NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{twoCommentsOnIdeaEvent, oneCommentOnIdeaEvent, baseBlogPost.getContent(), blogCommentUser1.getContent(), tempBlogCommentUser2.getContent()}, TEST_FILTERS, false);
		...
		// Verify that the one comment event, two comments event, create idea event, User 1's deleted comment and User 2's deleted comment are NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{twoCommentsOnIdeaEvent, oneCommentOnIdeaEvent, createIdeaEvent, tempBlogCommentUser1.getContent(), tempBlogCommentUser2.getContent()}, TEST_FILTERS, false);
	...
	/**
	 * <ul>
	 * <li><B>Name:</B> PrivateComIdeaCommentLike()</li>
	 * <li><B>Step:</B> testUser1 log in to Communities</li>
	 * <li><B>Step:</B> testUser1 open a community with private access that you have owner access to</li>
	 * <li><B>Step: testUser 1 add testUser 2 to the community as a member</B></li>
	 * <li><B>Step:</B> testUser1 add the ideablogs widget within this community</li>
	 ...
	 * <li><B>Step:</B> testUser2 log in to Connections</li>
	 * <li><B>Step:</B> testUser2 go to Home / Updates / Discover / All, Blogs & Communities</li>
	 * <li><B>Verify:</B> Verify that the news story for ideablog.comment.recommended is NOT seen</li>
	 * <li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/A1367D9D72E94E18852578760079EAB5">TTT - DISC - IDEABLOGS - 00070 - IDEABLOG.COMMENT.RECOMMENDED - PRIVATE COMMUNITY IDEABLOG (NEG)</a></li>	
	 * </ul>
	 * @author Naomi Pakenham
	 */
	@Test(groups = {"fvtonprem", "fvtcloud"}, priority = 4)
	public void PrivateComIdeaCommentLike() {
		...
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{likeCommentEvent, baseBlogPost.getContent(), baseBlogCommentUser1.getContent()}, TEST_FILTERS, false);
	...
	private void createBlogPostIfRequired() {
		if(blogPost == null) {
			...
			blogPost = CommunityBlogEvents.createIdea(restrictedCommunity, baseBlogPost, testUser1, communityBlogsAPIUser1);
		}
	}
