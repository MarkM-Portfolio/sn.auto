...
public class FVT_ImFollowing_PrivateFileCommentEdit_FileOverlay extends SetUpMethodsFVT {
	...
	*<li><B>Step: User 1 edit the comment</B></li>
	...
	*/
	...
	public void fileCommentEdited_FileOverlay(){
		...
		// User 1 will now comment on the private file and edit the comment
		...
		FileEvents.addAndEditFileComment(testUser1, filesAPIUser1, sharedFile, user1Comment, user1EditedComment);
		
		// Log in as User 2 and go to I'm Following
		LoginEvents.loginAndGotoImFollowing(ui, testUser2, false);
		
		// Create the news story required to open the file details overlay
		String commentEvent = FileNewsStories.getCommentOnTheirOwnFileNewsStory(ui, testUser1.getDisplayName());
		
		for(String filter : TEST_FILTERS) {
			// Ensure the relevant filter has been selected and 'Show More' clicked (if visible)
			UIEvents.filterBy(ui, filter);
			
			// Open the file details overlay for the news story
			FileEvents.openFileOverlay(ui, commentEvent);
			
			// Verify that the file details overlay displayed correctly
			boolean fileOverlayIsDisplayed = HomepageValid.verifyFileDetailsOverlayIsDisplayed(ui, baseFile);
			HomepageValid.verifyBooleanValuesAreEqual(fileOverlayIsDisplayed, true);
		}
		ui.endTest();
	}
}