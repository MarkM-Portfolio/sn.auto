...
public class FVT_YourEntry_EditComment_Others_Public extends SetUpMethodsFVT {
	...
	*<li><B>Step: User 3 comment on the entry and edit the comment</B></li>
	*<li><B>Step: User 1 go to Homepage / My Notifications / All, Blogs, Communities - verification point 2</B></li>
	*<li><B>Step: User 4 to User 8 comment on the entry and edit the comments</B></li>
	...
	*<li><B>Verify: Verify the event shows "{user 3} and {user 2} commented on your {entryName} entry in the{blogName} blog."</B></li>
	*<li><B>Verify: Verify the event shows "{user 8} and 6 others commented on your {entryName} entry in the {blogName} blog."</B></li>
	*<li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/C22D63C529E2BF4585257DE8003C5E78">TTT - MY NOTIFICATIONS - BLOGS - 00020 - UPDATE COMMENT ON ENTRY ROLLS UP</a></li>
	*</ul>
	*/
	...
	public void test_YourEntry_CommentUpdate_Rollup(){
		...
		String commentEvent = CommunityBlogNewsStories.getCommentOnYourEntryNewsStory_User(ui, baseBlogPost.getTitle(), baseCommunity.getName(), testUser2.getDisplayName());
		String updateCommentEvent = CommunityBlogNewsStories.getUpdateCommentOnYourEntryNewsStory_User(ui, blogPost.getTitle(), baseCommunity.getName(), testUser2.getDisplayName());
		...
		// Create the news stories to be verified
		commentEvent = CommunityBlogNewsStories.getCommentOnYourEntryNewsStory_TwoUsers(ui, testUser2.getDisplayName(), baseBlogPost.getTitle(), baseCommunity.getName(), testUser3.getDisplayName());
		...
		for(String filter : TEST_FILTERS) {
			// Verify that the comment event and User 2 and User 3's updated comments are displayed in all views
			...
			// Verify that the update comment event and User 3's original comment are NOT displayed in any of the views
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{updateCommentEvent, baseBlogCommentUser3.getContent()}, null, false);
		}
		
		// Users 4 through to 8 will now add comments to the blog entry and will edit the comments
		...
		for(int index = 0; index < baseBlogComments.length; index ++) {
			CommunityBlogEvents.createCommentAndEditComment(blogPost, baseBlogComments[index], updatedBaseBlogComments[index], usersPostingComments[index], apiUsersPostingComments[index]);
		}
		
		// Create the news stories to be verified
		commentEvent = CommunityBlogNewsStories.getCommentOnYourEntryNewsStory_UserAndMany(ui, testUser8.getDisplayName(), "6", baseBlogPost.getTitle(), baseCommunity.getName());
		updateCommentEvent = CommunityBlogNewsStories.getUpdateCommentOnYourEntryNewsStory_User(ui, baseBlogPost.getTitle(), baseCommunity.getName(), testUser8.getDisplayName());
		
		for(String filter : TEST_FILTERS) {
			// Verify that the comment event and updated comments posted by Users 7 and 8 are displayed in all views
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{commentEvent, baseBlogPost.getContent(), updatedBaseBlogComments[4].getContent(), updatedBaseBlogComments[3].getContent()}, filter, true);
			
			// Verify that the update comment event and none of the other comments are displayed in any of the views
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{updateCommentEvent, baseBlogComments[4].getContent(), baseBlogComments[3].getContent()}, null, false);
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{baseBlogComments[0].getContent(), baseBlogComments[1].getContent(), baseBlogComments[2].getContent()}, null, false);
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{updatedBaseBlogComments[0].getContent(), updatedBaseBlogComments[1].getContent(), updatedBaseBlogComments[2].getContent()}, null, false);
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{updatedBaseBlogCommentUser2.getContent(), updatedBaseBlogCommentUser3.getContent()}, null, false);
		}
		ui.endTest();
	}
}