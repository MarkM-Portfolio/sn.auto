...
public class FVT_YourWikiPage_EditComment_Others extends SetUpMethodsFVT {
	...
	/**
	* test_YourWikiPage_EditComment_Rollup() 
	*<ul>
	...
	*<li><B>Step: User 2 comment on the wiki page User 1 added and edit the comment</B></li>
	*<li><B>Step: User 1 go to Homepage / My Notifications / All, Wikis, Communities - verification point 1</B></li>
	*<li><B>Step: User 3 comment on the wiki page and edit the comment</B></li>
	*<li><B>Step: User 1 go to Homepage / My Notifications / All, Wikis, Communities - verification point 2</B></li>
	*<li><B>Step: User 4 comment on the wiki page and edit the comment</B></li>
	*<li><B>Step: User 1 go to Homepage / My Notifications / All, Wikis, Communities - verification point 3</B></li>
	*<li><B>Verify: Verify the event shows "<user 2> commented on your wiki page Re: <wikiPageName> in the <wikiName> wiki"</B></li>
	*<li><B>Verify: Verify the event shows "<user 3> and <user 2> commented on your wiki page Re: <wikiPageName> in the <wikiName> wiki"</B></li>
	*<li><B>Verify: Verify the event shows "<user 4> and 2 others commented on your wiki page Re: <wikiPageName> in the <wikiName> wiki"</B></li>
	*<li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/DD06E58861BE661E85257DEA004B6EEE">TTT - MY NOTIFICATIONS - WIKIS - 00030 - UPDATE COMMENT ON WIKI PAGE ROLLS UP</a></li>
	*</ul>
	*/
	@Test(groups = {"fvtonprem", "fvtcloud"})
	public void test_YourWikiPage_EditComment_Rollup(){
		...
		// User 2 will now post a comment to the wiki page and will edit the comment
		...
		for(String filter : TEST_FILTERS) {
			// Verify that the comment on wiki page event and User 2's updated comment are displayed in all views
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{commentOnWikiPageEvent, baseWikiPage.getDescription(), user2EditedComment}, filter, true);
		...
		// User 3 will now post a comment to the wiki page and will edit the comment
		...
		for(String filter : TEST_FILTERS) {
			// Verify that the comment on wiki page event and User 2's and User 3's updated comments are displayed in all views
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{commentOnWikiPageEvent, baseWikiPage.getDescription(), user2EditedComment, user3EditedComment}, filter, true);
		...
		// User 4 will now post a comment to the wiki page and will edit the comment
		...
		// Create the news story to be verified
		commentOnWikiPageEvent = CommunityWikiNewsStories.getCommentOnYourWikiPageNewsStory_UserAndMany(ui, testUser4.getDisplayName(), "2", baseWikiPage.getName(), baseCommunity.getName());
		
		for(String filter : TEST_FILTERS) {
			// Verify that the comment on wiki page event and User 3's and User 4's updated comments are displayed in all views
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{commentOnWikiPageEvent, baseWikiPage.getDescription(), user3EditedComment, user4EditedComment}, filter, true);
			
			// Verify that User 2's, User 3's and User 4's original comments and User 2's updated comment are NOT displayed in any of the views
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{user2Comment, user2EditedComment, user3Comment, user4Comment}, null, false);
		}
		ui.endTest();
	}
}