...
public class FVT_Saved_Public_StandaloneActivities extends SetUpMethodsFVT {
	
	private final String[] TEST_FILTERS = { HomepageUIConstants.FilterAll, HomepageUIConstants.FilterActivities };
	
	private Activity publicActivity, publicActivityUser3;
	private APIActivitiesHandler activitiesAPIUser1, activitiesAPIUser2, activitiesAPIUser3, activitiesAPIUser4;
	private APIProfilesHandler profilesAPIUser1, profilesAPIUser3, profilesAPIUser4;
	private BaseActivity baseActivity, baseActivityUser3;
	...
	public void setUpClass() {
		...
		// User 1 and User 2 will now follow User 1's public activity
		APIActivitiesHandler[] activityFollowers = { activitiesAPIUser1, activitiesAPIUser2 };
		ActivityEvents.addFollowerMultipleUsers(publicActivity, activityFollowers);
		
		// User 3 will now create a public standalone activity
		baseActivityUser3 = ActivityBaseBuilder.buildBaseActivity(getClass().getSimpleName() + Helper.genStrongRand(), false);
		publicActivityUser3 = ActivityEvents.createActivity(testUser3, activitiesAPIUser3, baseActivityUser3, isOnPremise);
		
		// User 3 and User 4 will now follow User 3's public activity
		ActivityEvents.addFollowerSingleUser(publicActivityUser3, activitiesAPIUser4);
	}
	
	@AfterClass(alwaysRun = true)
	public void performCleanUp() {
		
		// Delete the activities created for the test
		activitiesAPIUser1.deleteActivity(publicActivity);
		activitiesAPIUser3.deleteActivity(publicActivityUser3);
	}
	
	/**
	* test_PublicStandaloneActivity_MarkActivityStoriesAsSaved()
	*<ul>
	...
	*/
	...
	public void test_PublicStandaloneActivity_MarkActivityStoriesAsSaved() {
		...
		UIEvents.saveNewsStoryUsingUI(ui, makeActivityPublicEvent);
		
		// Navigate to the Saved view
		UIEvents.gotoSaved(ui);
		
		// Verify that the make activity public event is displayed in all views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{makeActivityPublicEvent, baseActivity.getGoal()}, TEST_FILTERS, true);
	...
	/**
	* test_PublicStandaloneActivity_RemovingActivityStoriesFromSaved()
	*<ul>
	*<li><B>Info: It can be assumed that each testcase starts with login and ends with logout and the browser being closed</B></li>
	...
	*<li><b>6: Go to Homepage / Saved / Activities
	...
	*/
	@Test(groups = {"fvtonprem", "fvtcloud"}, priority = 1)
	public void test_PublicStandaloneActivity_RemovingActivityStoriesFromSaved() {
		...
		// Create the news story to be saved
		String makeActivityPublicEvent = ActivityNewsStories.getMakeActivityPublicNewsStory(ui, baseActivityUser3.getName(), testUser3.getDisplayName());
				
		if(isOnPremise) {
			// Save the make activity public event using the API
			ProfileEvents.saveNewsStory(profilesAPIUser3, makeActivityPublicEvent, true);
				
			// Log in as User 1
			LoginEvents.loginToHomepage(ui, testUser3, false);
		} else {
			// Log in as User 1 and go to the Discover view
		...
		// Verify that the make activity public event is displayed in all views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{makeActivityPublicEvent, baseActivityUser3.getGoal()}, TEST_FILTERS, true);
		
		// Remove the make activity public news story using the UI
		UIEvents.removeNewsStoryFromSavedViewUsingUI(ui, makeActivityPublicEvent);
		
		// Verify that the make activity public event is NOT displayed in any of the views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{makeActivityPublicEvent, baseActivityUser3.getGoal()}, TEST_FILTERS, false);
		
		// Navigate to the Discover view
		UIEvents.gotoDiscover(ui);
		
		// Verify that the 'Save This' link is displayed on the make activity public event news story
		HomepageValid.verifyNewsStorySaveThisLinkIsDisplayed(ui, makeActivityPublicEvent);
		...
		// User 1 will now create an activity entry in the activity
		BaseActivityEntry baseActivityEntry = ActivityBaseBuilder.buildBaseActivityEntry(testName + Helper.genStrongRand());
		ActivityEntry activityEntry = ActivityEvents.createActivityEntry(testUser3, activitiesAPIUser3, baseActivityEntry, publicActivityUser3);
		
		// Create the news story to be saved
		String createEntryEvent = ActivityNewsStories.getCreateEntryNewsStory(ui, baseActivityEntry.getTitle(), baseActivityUser3.getName(), testUser3.getDisplayName());		
		...
		if(isOnPremise) {
			...
			// Log in as User 2
			LoginEvents.loginToHomepage(ui, testUser4, true);
		} else {
			// Log in as User 2 and go to the I'm Following view
			LoginEvents.loginAndGotoImFollowing(ui, testUser4, true);
		...
		// User 2 will now post a comment to the activity entry
		String user2Comment = Data.getData().commonComment + Helper.genStrongRand();
		ActivityEvents.createComment(publicActivityUser3, activityEntry, null, user2Comment, testUser4, activitiesAPIUser4, false);
		
		// Create the news story to be saved
		String commentOnEntryEvent = ActivityNewsStories.getCommentOnYourEntryNewsStory_User(ui, baseActivityEntry.getTitle(), baseActivityUser3.getName(), testUser4.getDisplayName());
		
		if(isOnPremise) {
			// Create the entry (API) version of the news story to be saved
			String commentOnEntryAPIEvent = ActivityNewsStories.getCommentOnEntryNewsStory(ui, baseActivityEntry.getTitle(), baseActivityUser3.getName(), testUser4.getDisplayName());
		
			// Save the comment on entry event using the API
			ProfileEvents.saveNewsStory(profilesAPIUser3, commentOnEntryAPIEvent, true);
			
			// Log in as User 1
			LoginEvents.loginToHomepage(ui, testUser3, true);
		} else {
			// Log in as User 1 and go to the My Notifications view
		...
		// Create the news story to be verified
		String savedCommentOnEntryEvent = ActivityNewsStories.getCommentOnTheEntryAddedByYouNewsStory(ui, baseActivityEntry.getTitle(), baseActivityUser3.getName(), testUser4.getDisplayName());
	...
	/**
	* test_PublicStandaloneActivity_SavingAnActivityEntry()
	*<ul>
	*<li><B>Info: It can be assumed that each testcase starts with login and ends with logout and the browser being closed</B></li>
	...
	*/
	...
	public void test_PublicStandaloneActivity_SavingAnActivityEntry() {
	...
	/**
	* test_PublicStandaloneActivity_StoryCommentedOnAfterSaving()
	*<ul>
	*<li><B>Info: It can be assumed that each testcase starts with login and ends with logout and the browser being closed</B></li>
	*<li><B>1: Log into Activities</B></li>
	*<li><B>2: Start a public activity</B></li>
	...
	*/
	@Test(groups = {"fvtonprem", "fvtcloud"}, priority = 2)
	public void test_PublicStandaloneActivity_StoryCommentedOnAfterSaving() {
		...
		ActivityEvents.createComment(publicActivity, activityEntry, null, user1Comment, testUser1, activitiesAPIUser1, false);
