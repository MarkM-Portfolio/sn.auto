...
public class FVT_1000Chars_ASComment extends SetUpMethodsFVT {
	...
	/**
	* addStatusUpdate_Comment1000_AS() 
	*<ul>
	...
	*<li><B>Step: Post a comment with 1000 characters on the status update from the Activity Stream</B></li>
	...
	*/
	...
	public void addStatusUpdate_Comment1000_AS(){
		...
		// User 1 will now post a comment with 1000 characters to their status update
		String user1Comment = Data.getData().Chars1000.replace('0', 'o');
		...
		for(String filter : TEST_FILTERS) {
			...
			String showMoreCSSSelector;
			if(filter.equals(HomepageUIConstants.FilterAll)) {
				showMoreCSSSelector = HomepageUIConstants.ShowMoreCommentContent_Unique.replace("PLACEHOLDER", user1StatusUpdate);
			} else {
				showMoreCSSSelector = HomepageUIConstants.ShowMoreCommentContent_Unique.replace("PLACEHOLDER", commentOnMessageEvent);
			}
