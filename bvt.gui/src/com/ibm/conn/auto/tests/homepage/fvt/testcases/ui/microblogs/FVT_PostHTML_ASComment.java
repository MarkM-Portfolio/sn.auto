...
public class FVT_PostHTML_ASComment extends SetUpMethodsFVT {
	...
	*<li><B>Step: Post a comment with HTML code on the status update from the Activity Stream</B></li>
	...
	*/
	...
	public void addStatusUpdate_CommentHTML_AS(){
		...
		// User 1 will now post a comment to the status update which includes HTML content
		String user1CommentWithHTML = Data.getData().commonComment + Helper.genStrongRand() + " " + Data.getData().htmlContent;
		ProfileEvents.addStatusUpdateCommentUsingUI(ui, testUser1, user1StatusUpdate, user1CommentWithHTML);
		...
		for(String filter : TEST_FILTERS) {
			...
			HomepageValid.verifyItemsInAS(ui, driver, new String[]{user1StatusUpdate, user1CommentWithHTML}, null, true);
		}
		ui.endTest();
	}
}