...
public class FVT_PostLinks extends SetUpMethodsFVT {
	...
	*<li><B>Step: Post a status update with a link in the sharebox</B></li>
	...
	*/
	...
	public void addStatusUpdateLink_AS(){
		...
		// User 1 will now post a status update which includes a link
		String user1StatusUpdateWithLink = Data.getData().commonStatusUpdate + Helper.genStrongRand() + " " + Data.getData().utvIrlURL + " ";
		ProfileEvents.addStatusUpdateUsingUI(ui, testUser1, user1StatusUpdateWithLink, true);
		
		// Verify that User 1's status update with links is displayed in all views
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{user1StatusUpdateWithLink.trim()}, TEST_FILTERS, true);
	...
	*<li><B>Step: Post a comment with a link on the status update in the EE</B></li>
	...
	*/
	@Test(groups = {"fvtonprem", "fvtcloud"})
	public void addStatusUpdate_CommentLink_EE(){
		...
		// User 1 will now post a comment to the status update which includes a link using the EE
		String user1CommentWithLink = Data.getData().commonComment + Helper.genStrongRand() + " " + Data.getData().skyNewsURL + " ";
		UIEvents.addEECommentUsingUI(ui, testUser1, user1StatusUpdate, user1CommentWithLink);
		...
		HomepageValid.verifyItemsInASUsingMultipleFilters(ui, driver, new String[]{commentOnMessageEvent, user1StatusUpdate, user1CommentWithLink.trim()}, TEST_FILTERS, true);
