...
public class FVT_VisitorModel_EE_Comment_StatusUpdate_NoVisitor extends SetUpMethods2{
	private static Logger log = LoggerFactory.getLogger(FVT_VisitorModel_EE_Comment_StatusUpdate_NoVisitor.class);
	...
	/**
	* visitor_ee_comment_privateCommunity_noVisitor_statusUpdateComment() 
	*<ul>
	...
	*<li><B>Step: testUser1 adds a comment to the community status update</B></li>
	*<li><B>Step: testUser3 log into Homepage / Updates / I'm Following / All</B></li>
	*<li><B>Step: testUser3 goes to the story of the Community Status Update comment</B></li>
	...
	*<li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/1E1E2292FCF6F4F685257C8900465A9A">TTT - VISITORS NOT ADDED TO COMMUNITY -EE - 00041 - PRIVATE COMMUNITY.STATUS.UPDATE.COMMENTED</a></li>
	*</ul>
	*/
	@Test(groups = {"fvtvisitor"})
	public void visitor_ee_comment_privateCommunity_noVisitor_statusUpdateComment() {
		
		String testName = ui.startTest();
		
		log.info("INFO: " + testUser1.getDisplayName() + " creates a restricted community");
		BaseCommunity baseCommunity = CommunityBaseBuilder.buildBaseCommunity(testName + Helper.genStrongRand(), Access.RESTRICTED);
		Community restrictedCommunity = executeCommonCreateAndFollowCommunitySteps(baseCommunity);
		...
		String communityStatusUpdateId = communityAPIUser1.addStatusUpdate(restrictedCommunity, communityStatusUpdate);
		
		log.info("INFO: " + testUser1.getDisplayName() + " adds a comment to the status update");
		String statusUpdateComment = Data.getData().commonComment + Helper.genStrongRand();
		String statusUpdateCommentId = communityAPIUser1.commentOnStatusUpdate(communityStatusUpdateId, statusUpdateComment);
		
		// Log into Connections as User 3
		executeCommonUser3LoginSteps();
		
		// Assign the news story to be clicked in order to open the EE
		String newsStory = ui.replaceNewsStory(Data.COMMUNITY_STATUS_UPDATE_COMMENT_SAME_USER, baseCommunity.getName(), null, testUser1.getDisplayName());
		...
		openEEAndExecuteAllCommonVerifications(newsStory, communityStatusUpdate, statusUpdateComment);
		
		log.info("INFO: Perform clean-up now that the test has completed");
		testUser1Profile.deleteSUComment(communityStatusUpdateId, statusUpdateCommentId);
