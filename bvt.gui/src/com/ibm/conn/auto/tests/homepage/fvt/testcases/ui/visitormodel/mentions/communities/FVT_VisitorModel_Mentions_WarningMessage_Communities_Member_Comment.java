...
public class FVT_VisitorModel_Mentions_WarningMessage_Communities_Member_Comment extends SetUpMethods2{
	...
	/**
	* visitorModel_mentionsWarning_privateCommunity_members_comment() 
	*<ul>
	...
	*<li><B>Verify: Verify there is a mentions event in the view</B></li>
	*<li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/6E7648550579955C85257C8D0046D866">TTT - MENTIONS - 00024 - VISITORS - NO WARNING MESSAGE WHEN VISITOR CANT BE MENTIONED - PRIVATE COMMUNITY COMMENT - MEMBER</a></li>
	*</ul>
	*/
	@Test(groups = {"fvtvisitor"})
	public void visitorModel_mentionsWarning_privateCommunity_members_comment() {
		...
		// User 1 will now post a comment with mentions to the community status update - mentioning a visitor who is a member 
		boolean commentPostedCorrectly = CommunityEvents.loginAndNavigateToCommunityAndAddStatusUpdateCommentWithMentions(restrictedCommunity, baseCommunity, ui, driver, uiCo, statusUpdate, testUser1, communitiesAPIUser1, mentions, false, false);
		
		// Verify that the comment with mentions posted correctly and warning messages were NOT displayed at any point during the process
		 ...
		 *  
		 *  		In order to cover the same verifications as before, this selector is now used twice:
		 *   			1) Check to verify that the more generic version of the element is visible in the UI. 
		 *   			2) Check to verify that the text inside of that element is "Shared externally" as expected.
		 */
		...
		HomepageValid.verifyItemsInAS(ui, driver, new String[]{mentionedYouEvent, statusUpdate, mentionsText}, null, true);
		...
		// Verify that the text inside of the Shared externally element is correct in the UI
		HomepageValid.verifyStringValuesAreEqual(driver.getFirstElement(messageCSSSelector).getText().trim(), "Shared externally");
