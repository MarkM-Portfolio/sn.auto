...
public class FVT_VisitorModel_SharedExternallyHeader_FileEvents_NoVisitor extends SetUpMethods2{
	...
	private Community restrictedCommunityUser1, restrictedCommunityUser3;
	...
	public void setUpClass() {
		...
		// User 1 will now create the restricted community with User 3 added as a member and a follower
		baseCommunityUser1 = CommunityBaseBuilder.buildBaseCommunity(getClass().getSimpleName() + Helper.genStrongRand(), Access.RESTRICTED);
		restrictedCommunityUser1 = CommunityEvents.createNewCommunityWithOneMemberAndOneFollower(baseCommunityUser1, testUser3, communitiesAPIUser3, testUser1, communitiesAPIUser1);
	
		// User 3 (acting as User 1) will now create the restricted community with User 1 added as a member and a follower
		baseCommunityUser3 = CommunityBaseBuilder.buildBaseCommunity(getClass().getSimpleName() + Helper.genStrongRand(), Access.RESTRICTED);
		restrictedCommunityUser3 = CommunityEvents.createNewCommunityWithOneMemberAndOneFollower(baseCommunityUser3, testUser1, communitiesAPIUser1, testUser3, communitiesAPIUser3);
	}
	...
	public void performCleanUp() {
		...
		communitiesAPIUser1.deleteCommunity(restrictedCommunityUser1);
		communitiesAPIUser3.deleteCommunity(restrictedCommunityUser3);
	}

	/**
	* visitor_sharedExternallyHeader_privateCommunity_noVisitor_fileAdded() 
	*<ul>
	...
	*<li><B>Step: testUser2 log into Homepage / Updates / I'm Following / Communities - verification point</B></li>
	*<li><B>Verify: 'Shared externally' header in a yellow background DOES NOT appear beside the event title</B></li>
	*<li><a HREF="Notes://CAMDB01/85257863004CBF81/A3B1F5A7FAF7FB158525703C006F870C/404A4CE4B5D7456C85257C8A0043DBBE">TTT - VISITORS - ACTIVITY STREAM - 00070 - SHARED EXTERNALLY HEADER - FILE EVENTS- PRIVATE COMMUNITY - VISITOR NOT ADDED</a></li>
	*</ul>
	*/
	@Test(groups = {"fvtvisitor"})
	public void visitor_sharedExternallyHeader_privateCommunity_noVisitor_fileAdded() {
		...
		CommunityFileEvents.addFile(restrictedCommunityUser1, baseFile, testUser1, filesAPIUser1);
		...
		// Verify that the 'Shared Externally' header is NOT displayed with the file shared event in any of the views
		HomepageValid.verifySharedExternallyHeader(ui, driver, fileSharedEvent, null, null, null, TEST_FILTERS, false);
		
		ui.endTest();
	}

	/**
	* visitor_sharedExternallyHeader_privateCommunity_noVisitor_fileComment() 
	*<ul>
	*<li><B>Info: It can be assumed that each testcase starts with login and ends with logout and the browser being closed</B></li>
	*<li><B>Step: testUser1 log in to Communities</B></li>
	*<li><B>Step: testUser1 creates a private community adding testUser3 as a member</B></li>
	...
	*/
	@Test(groups = {"fvtvisitor"})
	public void visitor_sharedExternallyHeader_privateCommunity_noVisitor_fileComment() {
		
		ui.startTest();
		
		// User 1 will now add a file to the community
		BaseFile baseFile = FileBaseBuilder.buildBaseFile(Data.getData().file2, ".jpg", ShareLevel.EVERYONE);
		FileEntry communityFile = CommunityFileEvents.addFile(restrictedCommunityUser1, baseFile, testUser1, filesAPIUser1);
		
		// User 1 will now comment on the file
		String commentOnFile = Data.getData().commonComment + Helper.genStrongRand();
		CommunityFileEvents.addComment(restrictedCommunityUser1, communityFile, commentOnFile, testUser1, filesAPIUser1);
		...
		// Verify that the 'Shared Externally' header is NOT displayed with the comment on file event in any of the views
		HomepageValid.verifySharedExternallyHeader(ui, driver, fileCommentEvent, commentOnFile, null, null, TEST_FILTERS, false);
		
		ui.endTest();
	}

	/**
	* visitor_sharedExternallyHeader_privateCommunity_noVisitor_fileComment_mention() 
	*<ul>
	*<li><B>Info: It can be assumed that each testcase starts with login and ends with logout and the browser being closed</B></li>
	*<li><B>Step: testUser1 log in to Communities</B></li>
	*<li><B>Step: testUser1 creates a private community adding testUser3 as a member</B></li>
	...
	*/
	@Test(groups = {"fvtvisitor"})
	public void visitor_sharedExternallyHeader_privateCommunity_noVisitor_fileComment_mention() {
		...
		// User 1 will now add a file to the community
		BaseFile baseFile = FileBaseBuilder.buildBaseFile(Data.getData().file1, ".jpg", ShareLevel.EVERYONE);
		FileEntry communityFile = CommunityFileEvents.addFile(restrictedCommunityUser3, baseFile, testUser3, filesAPIUser3);
		...
		CommunityFileEvents.loginAndNavigateToCommunityFiles(restrictedCommunityUser3, baseCommunityUser3, ui, uiCo, testUser1, communitiesAPIUser1, false);
		
		// User 1 will now comment on the file with mentions to User 3
		CommunityFileEvents.addCommentWithMentions(restrictedCommunityUser3, communityFile, mentions, testUser3, filesAPIUser3);
		...
		// Verify that the 'Shared Externally' header is NOT displayed with the comment on file event in any of the views
		HomepageValid.verifySharedExternallyHeader(ui, driver, fileCommentEvent, mentionsComment, null, null, TEST_FILTERS, false);
