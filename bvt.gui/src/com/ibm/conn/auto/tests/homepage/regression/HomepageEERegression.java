...
public class HomepageEERegression extends SetUpMethods2{
	
	private static Logger log = LoggerFactory.getLogger(HomepageEERegression.class);
	 ...
	 * Passing tests from the current Embedded Experience (EE) regression suite have been copied into this file.
	...
	*/
	...
	public void Blogs_EE_Basic() throws Exception {
		...
		//add widget
		logger.strongStep("Add a blog widget to the community using API");
		...
		ui.loginAndToggleUI(commFollower,cfg.getUseNewUI());
		
		//Open Update menu item
		logger.strongStep("Open 'Update' menu item");
		ui.gotoUpdates();
		
		//Select following tab
		logger.strongStep("Select 'Following' tab");
	...
	*/
	...
	public void Blogs_EE_Likes() throws Exception {
		...
		//allocate users
		User	communityFollower = cfg.getUserAllocator().getUser();
		User	communityOwner = cfg.getUserAllocator().getUser();		
		User 	communityMember = cfg.getUserAllocator().getUser();
		
		//create API handlers
		APICommunitiesHandler apiOwner = new APICommunitiesHandler(serverURL, communityOwner.getAttribute(cfg.getLoginPreference()), communityOwner.getPassword());
		APICommunitiesHandler apiFollower = new APICommunitiesHandler(serverURL, communityFollower.getAttribute(cfg.getLoginPreference()), communityFollower.getPassword());
		...
		logger.strongStep("Load homepage and login as a community follower");
		ui.loadComponent(homepageURI);
		ui.loginAndToggleUI(communityFollower,cfg.getUseNewUI());
		...
		ui.loadComponent(homepageURI); 
		ui.loginAndToggleUI(communityOwner,cfg.getUseNewUI());
		
		//Open Update menu item
		logger.strongStep("Select 'Updates' from the left menu");
		log.info("Select updates from the left menu");		
		ui.gotoUpdates();
		
		//Select following tab
		logger.strongStep("Select the 'Following' tab");
		log.info("Select Following tab");		
		ui.gotoImFollowing();
		...
		Assert.assertTrue(ui.findPersonInLikeList(communityFollower));		
		Assert.assertTrue(ui.findPersonInLikeList(communityOwner));
	...
	*/
	...
	public void Blogs_EE_Comments() throws Exception {
		...
		//add widget
		logger.strongStep("Add blog widget to community using API");
		log.info("INFO: Add blog widget to community using API");
		if (apiOwner.getWidgetID(commAPI.getUuid(), "Blog").isEmpty()) 
		{
			community.addWidgetAPI(commAPI, apiOwner, BaseWidget.BLOG);
		...
		ui.loadComponent(homepageURI);
		ui.loginAndToggleUI(commOwner,cfg.getUseNewUI());
	...
	*<li><B>NOTE: regressioncloud flag removed because test fails in the cloud - needs investigation</li>
	*</ul>
	*@throws Exception
	*/
	...
	public void Files_EE_Basic() throws Exception {
		...
		//Open Update menu item
		logger.strongStep("Select 'Update' from the left");
		log.info("Select updates from the left menu");
		ui.gotoUpdates();
		...
		driver.getSingleElement(FilesUIConstants.FileOverlayMoreActions).click();
		...
		//Open Update menu item
		logger.strongStep("Select updated from the left menu");
		log.info("Select updates from the left menu");
		ui.gotoUpdates();
	...
	*<li><B>NOTE: regressioncloud flag removed, test fails when run in the cloud - needs investigation</li>
	*</ul>
	*@throws Exception
	*/
	@Test(groups = {"regression", "regressioncloud","cnx8ui-regression"} )
	public void Files_EE_Likes() throws Exception {
		...
		//Load component and login with community owner
		logger.strongStep("Load homepage and login as community owner");
		ui.loadComponent(homepageURI, true);
		ui.loginAndToggleUI(commOwner,cfg.getUseNewUI());
