...
ruby_block "output_dbs_root_parameter" do
  block do
    Chef::Log.warn("INFO: dbs_root parameter: #{node[:oracle][:rdbms][:dbs_root]}")
  end
  action :create
end

#directory "#{node[:oracle][:rdbms][:ora_home_12c]}/assistants/dbca/templates" do
#  owner 'oracle'
#  group 'oinstall'
#  mode '0755'
#  recursive true
...
node[:oracle][:rdbms][:dbs].each_key do |db|
  ...
  if node[:oracle][:rdbms][:dbbin_version] == "12c"
    ...
    if node[:oracle][:rdbms][:dbconsole][:emconfig]
      ...
      template "#{node[:oracle][:rdbms][:ora_home_12c]}/em.rsp" do
        owner 'oracle'
        group 'oinstall'
        mode '0644'
      end
      ...
      execute "set_sid_to_em-rsp_#{db}" do
        command "sed -i '2s/.*/SID=#{db}/' #{node[:oracle][:rdbms][:ora_home_12c]}/em.rsp"
        user 'oracle'
        group 'oinstall'
      end
      ...
      execute "conf_dbcontrol_#{db}" do
        command "export ORACLE_HOME=#{node[:oracle][:rdbms][:ora_home_12c]}; emca -config dbcontrol db -repos create -respFile #{node[:oracle][:rdbms][:ora_home_12c]}/em.rsp"
        user 'oracle'
        group 'oinstall'
        environment (node[:oracle][:rdbms][:env_12c])
      end
    ...
    execute "shred_em_rsp_#{db}" do
      command "/usr/bin/shred -zu #{node[:oracle][:rdbms][:ora_home_12c]}/em.rsp"
      user 'root'
      group 'root'
    end
