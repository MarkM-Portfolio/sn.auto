...
if [ -z "${buildLabel}" ]; then
	echo "EMMA: ERROR - buildLabel not defined."
	exit 1
fi

if [ -z "${buildDefinitionId}" ]; then
	echo "EMMA: ERROR - buildDefinitionId not defined."
	exit 1
fi

if [ -z "${buildRequesterUserId}" ]; then
	echo "EMMA: ERROR - buildRequesterUserId not defined."
	exit 1
fi
		
echo "EMMA: buildLabel: ${buildLabel}"
echo "EMMA: buildDefinitionId: ${buildDefinitionId}"
echo "EMMA: buildRequesterUserId: ${buildRequesterUserId}"
USER=`echo ${buildRequesterUserId} | cut -d '@' -f1`
echo "EMMA: USER: ${USER}"

if [ "${personalBuild}" == "true" -a "${TRACK_EMMA_RESULTS}" == "true" -a "${USER}" == "icci" ]; then		
	flag_trackEmmaResults=true
fi
...
if [ "${flag_trackEmmaResults}" == "true" ]; then		
	COVERAGE_HTML_ALL_BUILD_DEF_DIR=${COVERAGE_HTML_ALL_DIR}/${buildDefinitionId}
	COVERAGE_XML_ALL_BUILD_DEF_DIR=${COVERAGE_XML_ALL_DIR}/${buildDefinitionId}
	...
	COVERAGE_HTML_BUILD_DEF_DIR=${COVERAGE_HTML_ALL_DIR}/${buildDefinitionId}
	COVERAGE_XML_BUILD_DEF_DIR=${COVERAGE_XML_ALL_DIR}/${buildDefinitionId}

elif [ "${personalBuild}" == "true" ]; then		
	COVERAGE_HTML_BUILD_DEF_DIR=${EMMA_REPORTS_HTML_DIR}/personal-builds/${USER}/${buildDefinitionId}
	COVERAGE_XML_BUILD_DEF_DIR=${EMMA_REPORTS_XML_DIR}/personal-builds/${USER}/${buildDefinitionId}
else
	COVERAGE_HTML_BUILD_DEF_DIR=${EMMA_REPORTS_HTML_DIR}/misc-builds/${buildDefinitionId}
	COVERAGE_XML_BUILD_DEF_DIR=${EMMA_REPORTS_XML_DIR}/misc-builds/${buildDefinitionId}	
fi	
	
COVERAGE_HTML_COMPONENT_ROOT_DIR=${COVERAGE_HTML_BUILD_DEF_DIR}/${APPLICATION}_${buildLabel}
COVERAGE_HTML_FILE=${COVERAGE_HTML_COMPONENT_ROOT_DIR}/index.html
COVERAGE_XML_FILE=${COVERAGE_XML_BUILD_DEF_DIR}/${APPLICATION}_${buildLabel}.xml
		
echo "EMMA: Generating HTML (${COVERAGE_HTML_FILE}) and XML (${COVERAGE_XML_FILE}) reports..."
$EMMA_SCRIPT_DIR/emma_generate_reports.sh -i ${EMMA_METADATA_DIR}/${APPLICATION}.em,${COVERAGE_DATA_FILE_LIST} -h ${COVERAGE_HTML_FILE} -x ${COVERAGE_XML_FILE} ${CONFIG_FILE}
if [ $? != 0 ]; then
...
if [ "${flag_trackEmmaResults}" == "true" ]; then		
	...
	COVERAGE_HTML_LATEST_BUILD_DEF_DIR=${COVERAGE_HTML_LATEST_DIR}/${buildDefinitionId}
		
	COVERAGE_XML_LATEST_DIR=${EMMA_REPORTS_XML_DIR}/scheduled-builds/latest
	COVERAGE_XML_LATEST_BUILD_DEF_DIR=${COVERAGE_XML_LATEST_DIR}/${buildDefinitionId}
