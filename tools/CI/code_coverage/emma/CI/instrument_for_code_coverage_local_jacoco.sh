...
do
	...
	cp "${DIR_WITH_JARS_TO_INSTRUMENT[${index}]}/${JARS_TO_INSTRUMENT[${index}]}"  "${DIR_JARS_ORIGINAL}"
	if [ $? != 0 ]; then
	...
	wsbld -f ${JACOCO_HOME}/build_jacoco_instrument.xml instr
	if [ $? != 0 ]; then
		echo "Could not instrument jars in ${DIR_TO_INCLUDE}."
		exit 1
	fi
