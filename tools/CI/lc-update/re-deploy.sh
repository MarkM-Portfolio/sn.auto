...
Options:
  ...
  -T		use remote TDI when populating profiles. Only has effect when -p option is used.
...
while getopts DIrb:B:2XpPCcKT opt
do
    case $opt in
        b) BUILD="${OPTARG}";;
        B) STREAM="${OPTARG}";;
...
ssh $REMOTE_USER@$REMOTE_HOST "[ -d /tmp/LC_temp ] && umask 022 %% echo $BUILD_STR > /tmp/LC_temp/complete.txt"

if [ -n "$flag_populate_profiles" ]; then
    echo2 -n "Populate Profiles database ..."
	if [ -n "$flag_use_remote_tdi" ]; then
		echo2 -n " using remote TDI..."
