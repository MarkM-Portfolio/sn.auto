...
EAR_FILE_PATH=${CI_HOME}/${SRC}/${APPDIR}/build/blogs.ear/ear.prod/lib/${EAR_FILE_NAME}

JAR_FILE_NAME=blogs.test.jar
JAR_FILE_PATH=${CI_HOME}/${SRC}/${APPDIR}/build/blogs.test/bvt/lib/${JAR_FILE_NAME}

DEPLOY_SCRIPT_NAME=updt-blogs.py
DEPLOY_SCRIPT_PATH=${CI_HOME}/${DEPLOY_SCRIPT_NAME}

FILES_TO_COPY[1]=${EAR_FILE_PATH}
FILES_TO_COPY[2]=${JAR_FILE_PATH}
FILES_TO_COPY[3]=${DEPLOY_SCRIPT_PATH}
